...
const SellPage = () => {
  ...
  const processBarcode = useCallback((barcode) => {
    ...
    if (product) {
      // Correctly find the existing item in the cart by its unique barcode
    ...
    if (currentBarcodeInput) {
      ...
      const handleGlobalKeyDown = (e) => {
        if (document.activeElement === currentBarcodeInput) {
          // If a digit is pressed while barcode input is focused, append it
          if (e.key.match(/^\d$/)) {
            setBarcodeInput((prev) => prev + e.key);
            setScanFeedback(''); // Clear feedback as user is typing
          } else if (e.key === 'Enter') {
            e.preventDefault(); // Prevent default form submission behavior
            processBarcode(currentBarcodeInput.value);
            setBarcodeInput(''); // Clear input after processing
            setScanFeedback('');
            currentBarcodeInput.focus(); // Keep focus on the input
          }
        }
      };
  ...
  }, [processBarcode]); // Dependency on processBarcode is correct

  const handleBarcodeInputChange = (e) => {
    const value = e.target.value;
    setBarcodeInput(value);
    setScanFeedback('');
    // This handles actual scanner input which often sends the barcode + Enter quickly
    if (value.length === 13) {
      ...
      {checkoutSuccess && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-scale-in p-6 text-center">
            ...
            <h2 className="2xl font-bold text-green-600 mb-2">Checkout Successful!</h2>
